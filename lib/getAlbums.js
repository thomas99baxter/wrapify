const filterTopTracksToAlbum = (data) => {
  ...
  let resultUno = data.body.items.find((song) => song.album.name === Object.keys(result)[0])

  let filteredResult = {
    albumName: resultUno.album.name,
    albumCover: resultUno.album.images[0],
    songsFromAlbum: Object.values(result)[0],
    albumArtist: resultUno.album.artists[0].name,
  }
